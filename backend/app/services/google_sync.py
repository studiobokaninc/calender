...
def ensure_jst(dt: datetime | None) -> datetime | None:
    ...
    if dt.tzinfo is None:
        # Standardize on UTC naive for DB entries that are meant to be universal
        return pytz.utc.localize(dt).astimezone(jst)
...
def sync_task_to_google(db: Session, task: models.Task, token_row: models.UserGoogleToken, user_id: int):
    ...
    if is_offline:
        if sync_row:
            success = google_calendar.delete_calendar_event(
            ...
            if success or True:  # 失敗してもDBからは消して再試行を容易にする
    ...
    end_dt = task.due_date if task.due_date else (start_dt + timedelta(hours=1) if start_dt else datetime.utcnow())
    ...
    # Tasks are synced as all-day events as requested
    is_all_day = True
    ...
    if sync_row:
        ...
        if not success:
            logger.error(f"Failed to update task {task.id} in Google Calendar")
    else:
        # Create
...
def sync_event_to_google(db: Session, event: models.Event, token_row: models.UserGoogleToken, user_id: int):
    ...
    # オフライン時は削除
    if event.status == 'offline': # Eventは status カラムが online/offline を持つ
        if sync_row:
            google_calendar.delete_calendar_event(
                token_row.access_token, token_row.refresh_token, token_row.expires_at,
                sync_row.google_event_id, _ensure_calendar_id(db, token_row)
            )
            crud.delete_event_google_sync(db, user_id, event.id)
        return
