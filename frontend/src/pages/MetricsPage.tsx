...
import PersonalTaskLog from '../components/PersonalTaskLog'; //PersonalTaskLogとは、個人タスクの完了ログ・オンスケ率を表示するコンポーネントです。
...
const MetricsPage: React.FC = () => { //MetricsPageとは、メトリクスページを表示するコンポーネントです。
  ...
  const completedTasks = useMemo(() =>
    filteredTasks.filter(task => getTaskStatusCategory(task.status) === 'completed').length,
    [filteredTasks]
  );
  ...
  const inProgressTasks = useMemo(() =>
    filteredTasks.filter(task => {
      ...
      return (cat === 'in_progress' || cat === 'review') && !isOverdue(task);
    }).length,
    [filteredTasks]
  );

  const delayedTasks = useMemo(() =>
    filteredTasks.filter(task => isOverdue(task)).length,
    [filteredTasks]
  );
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 2 }}>
          <Box>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
              <BarChartIcon sx={{ fontSize: '2rem', color: '#00BCD4' }} />
              <Typography
                variant="h4"
                sx={{
                  fontWeight: 800,
      ...
      >
        ...
        >
          ...
          <Box sx={{ flex: 1, minHeight: 0, overflow: selectedTab === 4 ? 'hidden' : 'auto', overflowX: selectedTab === 4 ? 'hidden' : 'auto' }}>
            ...
            {/* タブ7: 個人タスクログ — 完了ログ・遅延日数・完了日・コスト合計・オンスケ率 */}
            {selectedTab === 7 && (
              <PersonalTaskLog tasks={tasks} users={users} projects={projects} />
            )}
          </Box>
        </Paper>
      </Box>
